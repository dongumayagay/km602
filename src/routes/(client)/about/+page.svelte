<svelte:head>
	<title>About | km602</title>
</svelte:head>


<main class="text-sky-700">

    <div class="hero min-h-screen lg:px-16 px-4">
        <div class="hero-content flex-col lg:flex-row">
            <img alt="header" src="/img/dreamer.png" class="lg:w-1/2 w-full">
            <div>
                <p class="font-semibold text-lg">WHO WE ARE</p>
                <p class="text-5xl leading-snug">We <span class="font-bold">Care </span>for your car</p>
                <p class="my-6 text-lg">At km602 Carwash, we understand the importance of keeping your car clean, well-maintained, and in top condition.</p>
            </div>
        </div>
    </div>




    <div class="hero min-h-screen bg-sky-50 lg:px-16 px-4">
        <div class="hero-content flex-col lg:flex-row-reverse">
            <img alt="header" src="/img/shop.jpg" class="lg:w-1/2 w-full">
            <div>
                <p class="text-5xl leading-snug">About <span class="font-bold">Us</span></p>
                <p class="my-6 text-lg">km602 Carwash, a car wash business founded in 2019. Our name is derived from the location of our establishment and we take pride in providing high-quality car wash services for various types of vehicles.</p>
                <p class="my-6 text-lg">We cater to a wide range of vehicles with different wash options available to suit your needs. For smaller vehicles like Jeepneys, motorcycles, and tricycles, we offer a quick and efficient pressure-only wash. Meanwhile, larger vehicles such as SUVs, sedans, and vans can opt for full wash, inner-only, or outer-only options.</p>
                <p class="my-6 text-lg">We value your time and convenience, which is why we offer appointments from 8:00 am to 6:00 pm with one-hour intervals to ensure we can accommodate your schedule. Our flexible scheduling allows you to easily fit a car wash into your busy day.</p>

            </div>
        </div>
    </div>





    <div class="hero min-h-screen lg:px-16 py-60 px-4">
        <div class="hero-content flex-col gap-y-32">

            <div class="flex-col lg:grid lg:grid-cols-2">
                <img alt="header" src="/img/vision.png" class="lg:w-full w-full">
                <div>
                    <!-- <p class="font-semibold text-lg">WHO WE ARE</p> -->
                    <p class="text-5xl leading-snug">Our <span class="font-bold">Mission</span></p>
                    <p class="my-6 text-lg">Our mission at km602 Carwash is to provide superior car wash services with exceptional customer service. Every visit to our car wash will be hassle-free and enjoyable.</p>
                </div>
            </div>
    
    
            <div class="flex-col lg:grid lg:grid-cols-2">
                
                <div class="lg:text-right lg:mt-16">
                    <p class="text-5xl leading-snug">Our <span class="font-bold">Vision</span></p>
                    <p class="my-6 text-lg">We envision a future where every car owner in the country can experience the high-quality car wash services that we provide.</p>
                </div>

                <img alt="header" src="/img/mission.png" class="lg:w-full w-full">
                
            </div>

        </div>

    </div>



    <!-- <div class="h-60"></div> -->

</main>